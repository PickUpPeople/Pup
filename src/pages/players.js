...
export const query = graphql`
  {
    allPrismicItem(filter: { data: { type: { eq: "Player" } } }) {
      nodes {
        data {
          type
          image {
...
const Players = ({ data }) => {
  const sortedPlayerData = shuffle(data.allPrismicItem.nodes);

  return (
    <Layout>
      <SEO title="Players" pageName="home" />
      ...
      <Scroller prismicdata={sortedPlayerData} />
